...
- Two steps, in this order: `pnpm migrate:new --name x` creates the migration without
  ...
  it sees, and pressing Enter there drops the vector indexes (it has happened twice).
  `migrate deploy` never diffs the schema, so it cannot generate anything.
...
- **Vector (hnsw) indexes are invisible to Prisma** — it has no such index type, so the
  ...
  emits `DROP INDEX` for them. It has already deleted them twice. So: `pnpm migrate:new`,
  delete any generated `DropIndex ..._hnsw` line from the file, then `pnpm migrate:deploy`.
  The opclass must match the query operator: `vector_cosine_ops` for `<=>` (sales search,
  FR-26), `vector_l2_ops` for `<->` (fraud RAG, FR-27).
