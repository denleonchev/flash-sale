 ...
 * (authoritative Postgres reads), HarnessResult (verdict + report), ConcurrencyHarness
...
import { ConcurrencyHarness } from "./harness.js";
...
async function main(): Promise<void> {
  ...
  try {
    const result = await new ConcurrencyHarness(prisma, config).run();
    result.print();
